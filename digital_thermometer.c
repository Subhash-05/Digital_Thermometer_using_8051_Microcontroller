...
sbit RS = P2^5; 
sbit RW = P2^6;
sbit EN = P2^7; //used control the LCD
sbit ale=P2^3;  
sbit oe=P2^4; 
sbit start=P2^1; 
sbit eoc=P2^0; 
sbit clk=P2^2;  //these controlls the ADC0808(ALE,OE,START,EOC.CLK)
...
str("!!welcome!!"); //Display welcome message
lcd_command(0x01);  // clear display
str("Temp:");     //Display Temp:
...
{
lcd_command(0x38); // 5x7 dot matrix
lcd_command(0x01); // clear display screen
lcd_command(0x0f); // cursor blinking ON
lcd_command(0x06); // cursor moves right after each character
lcd_command(0x0c); //cursor off
lcd_command(0x80); // move cursor to the first row and column
}
